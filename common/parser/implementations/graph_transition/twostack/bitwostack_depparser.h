#ifndef _BITWOSTACK_DEPPARSER_H
#define _BITWOSTACK_DEPPARSER_H

#include <vector>
#include <unordered_set>

#include "twostack_macros.h"
#include "common/parser/implementations/graph_transition/bi_two_stack_weight.h"
#include "common/parser/implementations/graph_transition/bigraph_transition_depparser.h"

namespace bitwostack {

	using ::DependencyGraph;
	using graph_transition::BiTwoStackWeight;
	using twostack::StateItem;
	using twostack::ActionConstant;
	using twostack::Action;
	...
	DepParser<RET_TYPE>::DepParser(const std::string & sInputFile, const std::string & sFeatureInput, const std::string & sFeatureOutput,
		...
		this->m_Weight = new BiTwoStackWeight<RET_TYPE, StateItem, ActionConstant>(sFeatureInput, sFeatureOutput,
	...
	inline void DepParser<RET_TYPE>::mem(const tscore & score) {
		this->m_abScores.insertItem(ScoredAction(Action::MEM, score + this->m_lPackedScore[Action::MEM]));
	}
	...
	inline void DepParser<RET_TYPE>::recall(const tscore & score) {
		this->m_abScores.insertItem(ScoredAction(Action::RECALL, score + this->m_lPackedScore[Action::RECALL]));
	}
	...
	void DepParser<RET_TYPE>::getOrUpdateFeatureScores(const StateItem & item, const ActionScoreIncrement & amount) {
		BiTwoStackWeight<RET_TYPE, StateItem, ActionConstant> * cweight = (BiTwoStackWeight<RET_TYPE, StateItem, ActionConstant>*)(this->m_Weight);
		cweight->getOrUpdateFeatureScores((BiGraphDepParserBase<RET_TYPE, StateItem, ActionConstant>*)this, item, item.secondStackBack() == -1 ? -1 : item.secondStackTop(), amount);
	}
}
