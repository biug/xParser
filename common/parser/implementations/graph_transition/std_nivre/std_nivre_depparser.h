...
namespace std_nivre {
	...
	void DepParser<RET_TYPE>::getOrUpdateFeatureScore(const StateItem & item, const AddScoreType & amount) {
		...
		if (m_bPath) {
			...
			cweight->m_mapSSTwN0ptSSTsynhw.getOrUpdateScore(m_lPackedScore, tri_features, m_nScoreIndex, amount, m_nTrainingRound);
			tri_features.refer(sst_word, n0_postag, sst_syn_head_word_postag.second());
			cweight->m_mapSSTwN0wSSTsynhpt.getOrUpdateScore(m_lPackedScore, tri_features, m_nScoreIndex, amount, m_nTrainingRound);
			tri_features.refer(n0_word, sst_syn_head_word_postag.first(), sst_postag);
			cweight->m_mapSSTptN0wSSTsynhw.getOrUpdateScore(m_lPackedScore, tri_features, m_nScoreIndex, amount, m_nTrainingRound);
			tri_features.refer(sst_word, n0_syn_head_word_postag.first(), n0_postag);
			cweight->m_mapSSTwN0ptN0synhw.getOrUpdateScore(m_lPackedScore, tri_features, m_nScoreIndex, amount, m_nTrainingRound);
			tri_features.refer(sst_word, n0_postag, n0_syn_head_word_postag.second());
			cweight->m_mapSSTwN0wN0synhpt.getOrUpdateScore(m_lPackedScore, tri_features, m_nScoreIndex, amount, m_nTrainingRound);
			tri_features.refer(n0_word, n0_syn_head_word_postag.first(), sst_postag);
			cweight->m_mapSSTptN0wN0synhw.getOrUpdateScore(m_lPackedScore, tri_features, m_nScoreIndex, amount, m_nTrainingRound);
