#ifndef _BINIVRE_DEPPARSER_H
#define _BINIVRE_DEPPARSER_H
...
namespace binivre {
	...
	void DepParser<RET_TYPE>::getOrUpdateFeatureScores(const StateItem & item, const ActionScoreIncrement & amount) {
		...
		cweight->getOrUpdateFeatureScores((BiGraphDepParserBase<RET_TYPE, StateItem, ActionConstant>*)this, item, item.shiftBufferEmpty() ? -1 : item.shiftBufferTop(), amount);
	}
}
