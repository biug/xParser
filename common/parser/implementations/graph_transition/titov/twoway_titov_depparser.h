#ifndef _TWOWAY_TITOV_DEPPARSER_H
#define _TWOWAY_TITOV_DEPPARSER_H
...
namespace twoway_titov {
	...
	inline void DepParser<RET_TYPE>::swap(const tscore & score) {
		this->m_abScores.insertItem(ScoredAction(titov::SWAP, score + this->m_lPackedScore[titov::SWAP]));
	}
	...
	inline void DepParser<RET_TYPE>::reduce(const tscore & score) {
		this->m_abScores.insertItem(ScoredAction(titov::REDUCE, score + this->m_lPackedScore[titov::REDUCE]));
	}
	...
	inline void DepParser<RET_TYPE>::getActionScores(const StateItem & item) {
		...
		getOrUpdateFeatureScores(item, ActionScoreIncrement(titov::NO_ACTION, 0));
	}
