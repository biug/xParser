...
namespace arceagertag {
	...
	void DepParser::train(const DependencyTaggedTree & correct, const int & round) {
		// initialize
		int idx = 0;
	...
	void DepParser::parse(const Sentence & sentence, DependencyTaggedTree * retval) {
		int idx = 0;
		++m_nTrainingRound;
		DependencyTaggedTree correct;
		m_nSentenceLength = sentence.size();
		for (const auto & token : sentence) {
			m_lSentence[idx++].refer(TWord::code(SENT_WORD(token)), TPOSTag::code(SENT_POSTAG(token)));
			correct.push_back(DependencyTaggedTreeNode(DependencyTreeNode(token, -1, NULL_LABEL), NULL_LABEL));
		}
		work(retval, correct);
	...
	void DepParser::goldCheck(const DependencyTaggedTree & correct) {
		StateItem output;
		while (output.standardMove(correct)) {
		}
		int action;
		StateItem input;
		while (true) {
			action = input.followMove(output);
			input.move(action);
			if (action >= PP_FIRST && action < PP_END) {
				break;
			}
		}
		if (input != output) {
	...
	void DepParser::work(DependencyTaggedTree * retval, const DependencyTaggedTree & correct) {
		...
		for (int index = 0, len = (m_nSentenceLength << 1); index < len; ++index) {
			...
			if (m_nState == TRAIN) {
				...
				m_iCorrect.standardMove(correct);
			}
	...
	void DepParser::decode() {
		...
		for (const auto & iGenerator : *m_pGenerator) {
			...
			for (const auto & saScore : m_abScores) {
				...
				m_iCandidate.move(saScore->getAction(), m_mapPackedScore[saScore->getAction()]);
	...
	void DepParser::update() {
		...
		if (output != m_iCorrect) {
			while (m_iStatesItem != output) {
				int action = m_iStatesItem.followMove(output);
				int correct_action = m_iStatesItem.followMove(m_iCorrect);
	...
	void DepParser::generate(DependencyTaggedTree * retval, const DependencyTaggedTree & correct) {
		m_pGenerator->bestUnsortItem().generateTree(correct, *retval);
	}
