		...
		for (int d = 2; d <= m_nSentenceLength; ++d) {
			...
			for (int i = 0, max_i = m_nSentenceLength - d + 1; i < max_i; ++i) {
				...
				do {
					...
					//std::cout << "ritem score is " << m_lItems[d - 1][i + 1].r2l[GRAND_INDEX(i, i + 1, d - 1)].getScore() << std::endl;
					//std::cout << "l2r arc score is " << l2r_arc_score << std::endl;
					//std::cout << "bi sibling score is " << m_lBiSiblingArcScore[ENCODE_2ND_L2R(i, i)] << std::endl;
					//std::cout << "tri sibling score is " << triSiblingArcScore(i, -1, -1, l) << std::endl;
					//std::cout << "grand sibling score is " << m_lGrandSiblingArcScore[ENCODE_L2R(g)] << std::endl;
					//std::cout << "grand bi sibling score is " << grandBiSiblingArcScore(g, i, -1, l) << std::endl;
					//std::cout << "grand tri sibling score is " << grandTriSiblingArcScore(g, i, -1, -1, l) << std::endl;
					//std::cout << "best l2r score is " << item.l2r_solid_both[grand].bestItem().getScore() << std::endl;

					//std::cout << "grand is " << g << std::endl;
					//item.print(grand); //debug
