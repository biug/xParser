...
namespace eisnergc {
		...
		for (int d = 2; d <= m_nSentenceLength; ++d) {
			...
			for (int l = 0, n = m_nSentenceLength - d + 1; l < n; ++l) {
				...
				for (int m = l; m < r; ++m) {
					...
					for (int g = 0; g < l; ++g) {
						item.updateL2RIm(g, m, litem.l2r[g].getScore() + l2rPartialScore + m_lSecondOrderScore[ENCODE_2ND_L2R(l, g)]);
						item.updateR2LIm(g, m, ritem.r2l[g].getScore() + r2lPartialScore + m_lSecondOrderScore[ENCODE_2ND_R2L(l, g)]);
					}
					for (int g = r + 1; g <= m_nSentenceLength; ++g) {
						item.updateL2RIm(g, m, litem.l2r[g].getScore() + l2rPartialScore + m_lSecondOrderScore[ENCODE_2ND_L2R(l, g)]);
						item.updateR2LIm(g, m, ritem.r2l[g].getScore() + r2lPartialScore + m_lSecondOrderScore[ENCODE_2ND_R2L(l, g)]);
	...
	void DepParser::goldCheck() {
		Arcs2BiArcs(m_vecTrainArcs, m_vecTrainBiArcs);
		if (m_vecCorrectArcs.size() != m_vecTrainArcs.size() || m_lItems[m_nSentenceLength + 1][0].r2l[m_nSentenceLength].getScore() / GOLD_POS_SCORE != 2 * m_vecTrainArcs.size() - 1) {
