...
#include "arceagerpath_depparser.h"
...
namespace arceagerpath {
	...
	void DepParser::goldCheck(const DependencyPathTree & correct) {
		StateItem output;
		while (output.standardMove(correct)) {
		}
		int action;
		StateItem input;
		while ((action = input.followMove(output)) != POP_ROOT) {
			input.move(action);
		}
		input.move(action);
		if (input != output) {
	...
	void DepParser::work(DependencyTree * retval, const DependencyPathTree & correct) {
	...
	void DepParser::generate(DependencyTree * retval, const DependencyPathTree & correct) {
